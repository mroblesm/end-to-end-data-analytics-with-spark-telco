...
BQ_CONNECTOR_JAR_GCS_URI="gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.13-0.31.1.jar"
...
RUNTIME_PROPERTIES="^#^spark.jars.packages=ml.combust.mleap:mleap-spark-base_2.12:0.23.0,ml.combust.mleap:mleap-spark_2.12:0.23.0"
echo "RUNTIME_PROPERTIES : ${RUNTIME_PROPERTIES}"

${GCLOUD_BIN} dataproc batches submit pyspark ${PYSPARK_CODE} \
    --version=1.1 \
    --batch=${BATCH_UUID}  \
    --region=${GCP_REGION} \
    --history-server-cluster=${PERSISTENT_HISTORY_SERVER_NM} \
    --properties=${RUNTIME_PROPERTIES}  \
    --subnet=${SUBNET} \
    --service-account=${UMSA_FQN} \
    --container-image=${SPARK_CUSTOM_CONTAINER_IMAGE} \
    --jars=${BQ_CONNECTOR_JAR_GCS_URI} \
    -- \
    --pipelineID=${PIPELINE_ID} \
    --projectNbr=${PROJECT_NBR} \
